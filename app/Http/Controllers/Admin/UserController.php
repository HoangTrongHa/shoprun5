...
//        dd($request->all());
        ...
        return redirect()->back()->with("error","khong dung xin vui long..");
    }
    ...
    public function resetPassword(Request $request)
    {
        ...
        if (!$checkUser)
        {
            return redirect()->with("error","Xin lỗi đường dẫn không đúng vui lòng thử lại sau..");
        }
...
//        dd($request->all());
        $request->validate([
            ...
            [
                "password_cf.same"=>"Mật khâu không giống nhau"
            ]);
        if ($request->password)
        {
            $remember_token = $request->remember_token;
            ...
            if (!$checkUser)
            {
                return redirect()->back()->with("error","Xin lỗi đườ vui lòng thử lại sau..");
            }
            $checkUser->password = bcrypt($request->password);
            $checkUser->save();
            return redirect()->to(route("admin.login"))->with("success","Đổi mật khẩu thành công...");
